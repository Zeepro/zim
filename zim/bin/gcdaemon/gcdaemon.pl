...
sub analyze_file {
	...
	foreach my $line (@lines) {
		...
		# count of g1
		$pos_g1 = index($line, "M109");
		if ($pos_g1 != -1) {
			# do not count key word in comment
			unless ($pos_comment != -1 && $pos_comment < $pos_g1) {
				my $extruder_set = "";
				my $string_temp = "";
				my $offset = 0;
				
				# get temperature
				$pos_print = index ($line, 'E', $offset);
				while ($pos_print < $pos_g1) {
					$offset = $pos_print + 1;
					$pos_print = index ($line, 'E', $offset);
				}
				if ($pos_comment != -1 && $pos_comment > $pos_print) {
					$string_temp = substr($line, $pos_print + 1, $pos_comment - $pos_print - 1);
				}
				else {
					$string_temp = substr($line, $pos_print + 1);
				}
				
				# change only when we are in heating
				no warnings 'numeric';
				my $length_temp = $string_temp * 1;
